...
/mob/living/basic/carp/pet/lia
	name = "Лия"
	real_name = "Лия"
	desc = "Провальный эксперимент Trans-Solar Federation по созданию оружия из космокарпа. Этот менее устрашающий карп теперь служит питомцем для главы службы безопасности."
